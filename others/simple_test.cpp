...
const long interval = 3000; // 3 seconds in milliseconds
...
void loop()
{
    ...
    if (digitalRead(BUTTON_PIN) == LOW && !flags.wifi_flag)
    {
        Serial.println("Button pressed!");
        flags.wifi_flag = WifiManagerSetup();
        if (flags.wifi_flag)
            systemInit();
    ...
    if (currentMillis - previousMillis >= interval)
    {
        ...
        logData("This message is printed every 3 seconds");
    }
    // logData("Hello World");
}
